% function Params(im2, resultImage)
% % MSE Calculation
% 
% 
...
function Params(im2, resultImage,x)
% MSE Calculation
fprintf('PARAMETER VALUES');
   % Ensure both images are of the same size
    assert(all(size(im2) == size(resultImage)), 'Images must be of the same size.');
    ...
    if ~isequal(size(originalImage), size(noisyImage))
        % Resize the noisyImage to match the size of the originalImage
        noisyImage = imresize(noisyImage, size(originalImage));
    end
...
columnname = {'MSError','R.M.S.E','Edge strength','P.S.N.R','Accuracy','TruePositive','TrueNegative','FalsePositive','FalseNegative','Accuracy2','precesion','Recall','FalsePositiveRate','F1Score','EuclideanDistance'};
resultData = table(mseValue(:),rmse(:),es(:),PSNR(:),accuracy(:),truePositives(:),trueNegatives(:), ...
